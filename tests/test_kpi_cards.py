...
calculate_kpis / get_sparkline_data / get_sparkline_for_top_product /
...
def test_get_sparkline_for_top_product():
    df = pd.DataFrame(
        {
            "item_code": ["A", "A", "B"],
    ...
    spark = _kpi.get_sparkline_for_top_product(df, "A", days=7)
    assert len(spark) == 7
    assert spark[-2:] == [5, 7]  # only A's rows
    # sentinel top_item -> zeros
    assert _kpi.get_sparkline_for_top_product(df, "-", days=7) == [0] * 7
